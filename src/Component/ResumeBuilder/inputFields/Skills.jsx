...
const Skills = ({ skills, handleInputChange, addSkill, deleteSkill, skillsfromapi }) => {
  ...
  const handleAddSkill = () => {
    addSkill();
    setShowDropdown(false); // Collapse dropdown after clicking Add Item button
  };
  ...
  return (
    <div className="mt-10 px-5 sm:px-10 md:px-20 text-xs lg:text-sm">
      ...
      <div className="mt-4">
        ...
        {apiSkills.length > 0 ? (
          apiSkills.map((skill, index) => (
            <div key={index} className="flex flex-col md:flex-row items-center justify-center gap-4 bg-gray-50 p-4 rounded-lg shadow-md mb-4">
              <div className="w-full md:w-3/4 ">
                <label htmlFor={`skillname-api-${index}`} className="block text-sm font-medium text-gray-700 mb-2">
                  Skill Name
                </label>
      ...
      {skills.map((skill, index) => (
        <div key={index} className="mt-4">
          <div className="flex flex-col md:flex-row gap-4 bg-gray-50 p-4 rounded-lg shadow-md mb-4">
            <div className="w-full md:w-3/4  ">
              <label htmlFor={`skillname-${index}`} className="block text-sm font-medium text-gray-700 mb-2">
                Skill Name
              </label>
            ...
            <button type="button" onClick={() => deleteSkill(index)} className=" text-red-500">
              Delete Skill
            </button>
      ...
      <button className="mt-4 font-bold text-lg flex items-center bg-black text-white px-4 py-2 rounded-lg shadow hover:bg-green-600 transition duration-300" onClick={handleAddSkill}>
        <h3>Add Item</h3>
        <svg className="h-5 w-5 text-white bg-black rounded-full m-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          <path fill="none" d="M0 0h24v24H0z" />
          <line x1="9" y1="12" x2="15" y2="12" stroke="white" />
...
Skills.propTypes = {
  ...
  addSkill: PropTypes.func.isRequired,
  deleteSkill: PropTypes.func.isRequired,
  skillsfromapi: PropTypes.arrayOf(PropTypes.string).isRequired,
};
