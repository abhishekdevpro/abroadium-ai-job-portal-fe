...
//     <>
//       <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
//         ☰
//       </div>
...
const EnhancedTemplate3 = ({
  ...
  const renderSectionContent = (sectionName) => {
    switch (sectionName) {
      case "details":
        return (
          <div className="text-center flex flex-col justify-between gap-2">
            {details.map((del, idx) => (
              <React.Fragment key={idx}>
                <h3 className={`text-2xl font-bold text-gray-900 mb-2 ${font} ${textSize}`}>
                  {del.name || predefinedText.details.name}
                </h3>
                <p className={`text-lg text-gray-600 ${font} ${textSize}`}>
                  {del.Profession || predefinedText.details.Profession}
                ...
                <div className={`flex justify-between space-x-4 mt-2 text-md text-gray-700 ${font} ${textSize}`}>
                  ...
                  <a href={del.link || "#"} className="flex items-center space-x-2">
                    <FontAwesomeIcon icon={faLink} />
                    <span>{del.link || predefinedText.details.link}</span>
                  </a>
                  <a href={del.github || "#"} className="flex items-center space-x-2">
                    <FontAwesomeIcon icon={faGithub} />
                    <span>{del.github || predefinedText.details.github}</span>
                  </a>
                </div>
      ...
      case "summary":
        const summaryDescription = summary.length > 0
          ? summary[0].summarydescription
          : predefinedText.summary.summarydescription;
        return (
          <>
        ...
        return (
          <>
            ...
            {experiences.map((exp, idx) => (
              <div key={idx} className="mb-6">
                <div className="flex justify-between items-baseline">
                  <h3 className={`text-xl font-semibold text-gray-900 ${font} ${textSize}`}>
                    {exp.Company || predefinedText.experiences.company}
                  </h3>
                  <span className={`text-sm text-gray-600 ${font} ${textSize}`}>
                    {exp.month1} - {exp.month2}
      ...
      case "skills":
        return (
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              {skills.map((skill, idx) => (
                <div key={idx} className={`bg-gray-100 p-3 rounded ${font} ${textSize}`}>
                  <h3 className="font-semibold text-gray-800">
                    {skill.skillname || predefinedText.skills.skillname}
                  </h3>
                  <p className="text-sm text-gray-600">
      ...
      case "sectionadd":
        return (
          <>
            {sectionadd && sectionadd.length > 0 ? (
              sectionadd.map((sec, secIndex) => (
                <div key={`sec-${secIndex}`} className={`mb-6 ${paragraphSpacing} ${lineSpacing}`}>
                  <h6 className={`text-2xl font-bold mb-4 text-gray-800 ${font} ${textSize}`}>
                    {sec?.sectiontitle || predefinedText.sectionadd?.sectiontitle || "Section title not provided"}
                  </h6>
                  <div
  ...
  const renderSection = (sectionName, index) => {
    const sectionProps = {
      className: `pl-10 pr-6 py-6 rounded-lg shadow-md ${boxBgColor} ${paragraphSpacing} ${
        draggingSection === sectionName ? "opacity-50" : ""
      } relative group cursor-move hover:border-2 hover:border-blue-300 hover:bg-gray-50 transition-all duration-300`,
      onDragStart: (e) => handleDragStart(e, index),
      onDragEnter:
        draggingSection && draggingSection !== sectionName
          ? (e) => handleDragEnter(e, index)
